...
public class RequestRideController {
    ...
    /*stompClient.subscribe("/user/" + userId + "/queue/nearby-drivers", (message) => {
    const data = JSON.parse(message.body);
    console.log("Nearby drivers: ", data);
    showDriverOptions(data);
    });
     */
    ...
    public ResponseEntity<?> findNearbyDrivers(@Valid @RequestBody RideRequestDTO rideRequest) {
        ...
        log.info("Total drivers found in Redis: {}", allDriversMap.size());
        List<DriverLocationDTO> allDrivers = allDriversMap.values().stream()
                .map(obj -> (DriverLocationDTO) obj)
                .toList();
        if(allDrivers.isEmpty()){
            throw new APIException("No driver available at the moment, please try again later.");
        }
  ...
  //fare.replace("₹", "")
        ...
        messagingTemplate.convertAndSendToUser(
                userId.toString(), // this is the "username"
                "/queue/nearby-drivers", // destination after /user/
                responseList
        );

        // 6. Also return response for immediate client consumption (optional)
